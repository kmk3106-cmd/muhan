...
def _get_kis_filled_odnos(client: KISClient, ticker: str, today_str: str, ctac_tlno: str) -> set[str]:
    ...
    오판 시 동일 주문이 재제출되어 중복 매수 사고 발생(2026-07-08 종사종팔 UPRO 9주 중복 실사고).
...
def run_worker_once(submit_orders: bool = True):
    """떨사오팔 워커 1회 실행. submit_orders=False: 강제동기화 시 주문 제출 생략"""
    if not _worker_lock.acquire(blocking=False):
        logger.info("떨사오팔 워커: 다른 작업 실행 중 - 건너뜀")
...
def _run_worker_once_impl(submit_orders: bool = True):
    """워커 실제 로직 (락 획득 후 호출)"""
    logger.info("=" * 60)
    logger.info("떨사오팔 워커 실행 시작" + (" (동기화만)" if not submit_orders else ""))
