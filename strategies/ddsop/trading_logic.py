...
"""
떨사오팔 v1 - 트렌치 매매 로직
전일종가 -x% LOC매수, 평단가 +x% LOC매도, N거래일 손절 MOC매도 (N=loss_cut_days)
"""
import logging
import math
...
def generate_orders(
    ...
    """
    오늘 제출할 주문 목록 생성.
    1) BOUGHT 트렌치 → LOC매도 (평단가 +x%)
    2) N일 경과 BOUGHT 트렌치 → MOC매도 (손절, N=loss_cut_days)
    3) 다음 매수 대상 → LOC매수 (전일종가 -x%)
    ...
    if next_t:
        ...
        if buy_price > 0:
            buy_qty = max(1, int(amt_per / buy_price))
